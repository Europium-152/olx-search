...
def get_ads(search: str, contains: str=None, count: int=5, price_from: int=0, region: str=None) -> None:
    """Get OLX ads sorted by ascending price.

    Parameters
    ----------
    search: str
        Search string passed to OLX
    contains: str
        Filter ads that contain the *contains* string, ignoring white spaces and capitalization.
    count: int
        Number of ads to show
    price_from: int
        Lowest admissible price
    region: str
        Region of Portugal where to search. Some options are {'lisboa', 'porto', 'leiria'}. Refer to the olx website for more.
    """

    search_terms = [key for key in map(lambda x: x.strip(), search.lower().split(" ")) if key != '']
    search_string = "q-" + "-".join(search_terms)

    search_region = 'ads' if region is None else region

    search_URL = f"https://www.olx.pt/d/{search_region}/{search_string}/?search%5Border%5D=filter_float_price:asc&search%5Bfilter_float_price:from%5D={int(price_from)}"
